...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    const svcPorVisita   = totalClientes > 0 ? totalSvcs / totalClientes : 0; // solo servicios / clientes
    ...
    const workers = Object.values(workersMap).map(w => {
      ...
      const svcCli   = clientes > 0 ? (w.svcs / clientes).toFixed(1) : '0'; // solo servicios (sin productos)
      return {
        nombre:   w.nombre,
        role:     '',
        visitasR: clientes,
