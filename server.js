    ...
    invoices.forEach(inv => {
      ...
      if (usarTotalFactura) {
        ...
        items.forEach(item => {
          ...
          // Detectar si es producto: tiene label/etiqueta activada O tiene inventario
          const tieneLabel = item.label || item.productLabel || item.tag || false;
          const tieneInventario = item.trackInventory || false;
          const esProducto = !!(tieneLabel || tieneInventario);

          console.log('Item:', nombre, '| trabajadora:', trabajadora, '| esProducto:', esProducto, '| label:', tieneLabel, '| importe:', importe);
