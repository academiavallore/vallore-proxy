...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    const invoices = allInvoices.filter(i => i.status === 'paid');

    console.log('Total invoices a procesar:', invoices.length);

    // Imprimir JSON completo de la primera factura para debug
    if (invoices.length > 0) {
      console.log('PRIMERA FACTURA COMPLETA:', JSON.stringify(invoices[0]));
    }
    ...
    invoices.forEach(inv => {
      ...
      items.forEach(item => {
        ...
        const trabajadora = (item.description || item.notes || item.memo || '').trim() || 'Sin asignar';
    ...
    res.json({
      kpis: {
        visitasReales: 0, totalCitas: 0, totalSvcs: 0, svcPorVisita: 0, ticketMedio: 0,
      ...
      workers,
      productos: Object.entries(productosMap).map(([n, d]) => ({ n, qty: Math.round(d.qty), rev: Math.round(d.rev) })).sort((a, b) => b.rev - a.rev),
      categorias: Object.entries(categoriasMap).map(([n, d]) => ({ n, qty: Math.round(d.qty), rev: Math.round(d.rev) })).sort((a, b) => b.rev - a.rev),
    });
