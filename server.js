...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    invoices.forEach(inv => {
      ...
      totFacInv += paid;
      console.log('Factura:', inv._id, 'amountPaid:', paid);

      const items = inv.invoiceItems || inv.lineItems || inv.items || [];
      
      // Log completo del primer item para ver estructura
      if (items.length > 0) {
        console.log('Sample line item keys:', Object.keys(items[0]).join(', '));
        console.log('Sample line item full:', JSON.stringify(items[0]).slice(0, 600));
      }
