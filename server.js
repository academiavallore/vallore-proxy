...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    // 1. Citas — probamos múltiples endpoints hasta encontrar el correcto
    let appointments = [];
    const apptEndpoints = [
      `${GHL_BASE}/appointments/?locationId=${locationId}&startDate=${startDate}&endDate=${endDate}&limit=100`,
      `${GHL_BASE}/calendars/appointments?locationId=${locationId}&startDate=${startDate}&endDate=${endDate}&limit=100`,
      `${GHL_BASE}/calendars/events?locationId=${locationId}&startTime=${startDate}&endTime=${endDate}&limit=100`,
    ];
    for(const url of apptEndpoints) {
      console.log('Trying appointments endpoint:', url);
      const r = await fetch(url, { headers: {...headers, 'Version': '2021-07-28'} });
      ...
      if(r.status === 200) {
        ...
        try {
          ...
          const all = j.appointments || j.events || j.data || [];
          appointments = all.filter(e => ['showed','confirmed','completed'].includes(e.appointmentStatus || e.status || 'confirmed'));
          console.log('Appointments found:', appointments.length);
          break;
        } catch(e) { console.log('Parse error:', e.message); }
      } else {
        const txt = await r.text();
        console.log('Error response:', txt.slice(0, 200));
      }
    }

    // 2. Facturas pagadas — endpoint correcto
    ...
    console.log('Invoices response:', invText.slice(0, 500));
    let invJson = {};
    try { invJson = JSON.parse(invText); } catch(e) { console.log('Parse error invoices:', e.message); }
    ...
    // ── Procesar invoices — total facturado y productos inventario ──
    let totProds = 0, totFacProd = 0, totFacInv = 0;
    const prodMap = {};

    console.log('Total invoices a procesar:', invoices.length);
    invoices.forEach(inv => {
      // Sumar facturación total de la factura
      const paid = parseFloat(inv.amountPaid || inv.total || inv.amount || 0);
      totFacInv += paid;
      console.log('Factura:', inv._id, 'status:', inv.status, 'amountPaid:', paid);

      (inv.lineItems || inv.items || []).forEach(item => {
        // Productos de inventario tienen productId
    ...
    console.log('Total facturación invoices:', totFacInv, '| Productos:', totFacProd);
    ...
    res.json({
      kpis: {
        ...
        totalFacInv:    Math.round(totFacInv),
        totalFac:       Math.round(totFacServ + totFacProd + totFacInv),
      },
