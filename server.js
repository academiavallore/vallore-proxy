  ...
  try {
    ...
    invoices.forEach(inv => {
      ...
      items.forEach(item => {
        ...
        // Nombre trabajadora desde campo description (limpiando HTML)
        ...
        // Inicializar trabajadora
        if (!workersMap[trabajadora]) {
          workersMap[trabajadora] = {
            nombre:    trabajadora,
            facturas:  new Set(), // facturas (= clientes atendidos)
            svcs:      0,         // cantidad servicios
            facServ:   0,         // facturación servicios
            prod:      0,         // cantidad productos
            facProd:   0,         // facturación productos
            cats:      {},        // categorias servicio {nombre: {qty, rev}}
          };
        }

        // Añadir factura como cliente atendido por esta trabajadora
        ...
        if (esProducto) {
          ...
          if (!workersMap[trabajadora].cats[nombre]) {
            workersMap[trabajadora].cats[nombre] = { qty: 0, rev: 0 };
          }
