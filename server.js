...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    // 1. Citas showed — endpoint v2 calendars
    const apptUrl = `${GHL_BASE}/calendars/events?locationId=${locationId}&startTime=${startDate}&endTime=${endDate}&status=showed`;
    ...
    console.log('Appointments response:', apptText.slice(0, 300));
    let apptJson = {};
    try { apptJson = JSON.parse(apptText); } catch(e) { console.log('Parse error appointments:', e.message); }
    const appointments = apptJson.events || apptJson.appointments || apptJson.data || [];

    // 2. Facturas pagadas
    const invUrl = `${GHL_BASE}/invoices/?altId=${locationId}&altType=location&status=paid&limit=100`;
    ...
    console.log('Invoices response:', invText.slice(0, 300));
    let invJson = {};
